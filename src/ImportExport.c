        ...
        while (clientesHistorico)
        {
            ...
            for (int j = 0; j < numeroMaximoCaixasPossivel; j++)
            {
                ...
                while (caixasHistorico)
                {
                    ...
                    fprintf(arquivo, "%s;%d;%d;%s;%d;%d;%d;%d;%s;%s\n", clientesHistoricoInfo->nome, clientesHistoricoInfo->id, j + 1, caixasHistoricoInfo->funcionario->nome, caixasHistoricoInfo->funcionario->id, caixasHistoricoInfo->dataTransacao.dia, caixasHistoricoInfo->dataTransacao.mes, caixasHistoricoInfo->dataTransacao.ano, tempoCaixa, tempoAtraso);
